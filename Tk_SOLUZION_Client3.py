...
# Default problem name. Edit or give a command-line parameter to change it.
problem_name = 'Hunger'
#problem_name = 'Missionaries3'


def compute_applicability_vector():
    global CURRENT_STATE, APPLICABILITY_VECTOR
    APPLICABILITY_VECTOR = [op.is_applicable(
        CURRENT_STATE) for op in OPERATORS]
...
class lower_gui_part(tk.Frame):
    ...
    def update_choices(self):
        ...
        new_values = [(str(i)+": "+OPERATORS[i].name)
                      for i in range(len(OPERATORS)) if APPLICABILITY_VECTOR[i]]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
      ...
      ./IDLE_Text_SOLUZION_Client Missionaries
        ...
        exit(1)
        """
        sys.argv = ['Tk_SOLUZION_Client3.py',
                    problem_name]  # IDLE and Tk version only.
    ...
    # This lets Tk take over the event loop, show graphics, etc.
